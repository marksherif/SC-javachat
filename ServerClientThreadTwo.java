...
public class ServerClientThreadTwo extends Thread {
 Socket serverClient;
 int clientNo;
 ServerClientThreadTwo(Socket inSocket,int counter){
     ...
     while(!clientMessage.equals("quit")){
       ...
       if(clientMessage.startsWith("Join(") && clientMessage.endsWith(")")){
    	   ...
    	   if(Server.usersOne.contains(requestedUsername.toString()) || Server.usersTwo.contains(requestedUsername.toString())){
    		   ...
    		   Server.usersTwo.add(requestedUsername.toString());
    	       outStream.writeBytes(serverMessage +'\n');
    	       outStream.flush();
    	   }  
       }
    	   ...
    	   for(int i =0; i< Server.socketsTwo.size(); i++){
    		   if(Server.socketsTwo.get(i).equals(serverClient)){
    			   indexOfSourceSocket = i;
    			   i = Server.socketsTwo.size();
    		   } 
    	   }
    	   if (Server.usersTwo.contains(source)){
	    	   for(int i =0; i< Server.usersTwo.size(); i++){
	    		   if(Server.usersTwo.get(i).equals(source)){
	    			   indexOfUser = i;
    	   ...
    	   outStream.flush();
    		   ...
    		   Runnable mt = new MessageThread("ServerTwo", source, destination, Integer.parseInt(TTL), message, outStream);
    		   new Thread(mt).start();
    	   }
    	   }
       }
    	   ...
    	   Server.socketsTwo.remove(indexOfSourceSocket);
    	   Server.usersTwo.remove(indexOfSourceSocket);
